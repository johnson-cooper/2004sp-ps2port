...
- Last fully real-hardware-known-good integration commit: `a636ea11dbb3a90b062526f3e6135673ab70e043` (confirmed working on real PS2 on 2026-09-17; retains the qword-aligned 4 MiB scene arena, 32x32 / 1024-slot resident terrain window, fourteen-tile / 29x29 camera draw window, local-player model and bounded 32x32 static-loc window, restores normal dynamic chat/sidebar/tab UI composition, inventory item icons and interface 3D model components, and enables the controller virtual cursor without crashes. The cursor is visible over UI panels but is currently overwritten inside the game viewport by the temporary live-packet diagnostic viewport re-blit; fix that ordering/removal before adding another gameplay subsystem. Static loc/model rendering currently lowers observed live framerate from roughly 50 FPS to roughly 30 FPS; keep that performance cost visible while correctness restoration continues, with renderer/VU work treated as a separate later optimization effort.)
...
- Current policy: gameplay-first 32 MiB profile — untextured colored terrain, bounded/lazy Ground residency, fourteen-tile camera radius, local player enabled, bounded 32x32 static-loc placement enabled, capped nearby dynamic entities, normal interactive gameplay UI and inventory icons with simplified chrome, minimap off. The next isolated test removes the temporary live-packet viewport overlay that currently redraws the viewport after the cursor and hides the cursor only over the 3D game area.
