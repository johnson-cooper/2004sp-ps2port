...
- Last fully real-hardware-known-good integration commit: `a12c5c8c69b17fa329fe1371397fa90f9f8685fc` (accepted as the PS2 performance/stability baseline on real hardware on 2026-09-20). This baseline includes the prior hardware-stable scene/memory/render work and controller-native UX, plane-transition/chat readability fixes, projection reciprocal and sparse depth-bucket optimizations, divisor-2 / 25 Hz presentation, project-wide `-O1`, player LOD plus a nearest-first 32-player ordinary crowd budget, ground-item pile LOD, a bounded inactive-loc draw budget, player-anchored loc LOD that avoids camera-angle popping, the relaxed visibility wedge, and matching 80x80 terrain/static-loc residency for better scene-edge building coverage.
...
1. Preserve baseline `a12c5c8c69b17fa329fe1371397fa90f9f8685fc`: qword-aligned 80x80 terrain / 80x80 static-loc traversal, compact scene allocations, lazy interfaces, one-slot water texture, corrected picking, animated/occluded local player, normal gameplay UI, radius-18 camera-aware visibility with relaxed edge margins, player-anchored decorative-loc LOD, a 48-draw farther inactive-loc budget, ground-item pile LOD, distance-based player LOD plus a nearest-first 32 ordinary-player render budget, controller-native UX, bounded scene rebuilds on floor/plane changes, chat readability, model-projection reciprocal and sparse depth-bucket optimizations, project-wide `-O1`, and divisor-2 / 25 Hz presentation.
