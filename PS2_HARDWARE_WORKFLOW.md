...
- Last fully real-hardware-known-good integration commit: `d0aa55048ea99ebe66eab538040f3642414331cc` (confirmed working on real PS2 on 2026-09-17). Cooper stress-tested traversal around Lumbridge through repeated normal scene transitions without a crash. The accepted baseline is an 80x80 bounded terrain bridge window, a lighter 72x72 static-loc/model window, radius-14 / 29x29 camera window, qword-aligned 4 MiB scene arena, one-slot low-memory terrain texturing with water visible, local player, gameplay UI, inventory icons and viewport/UI virtual cursor.
...
- Current world-restoration issues: PS2 viewport model picking is inaccurate, especially noticeable around trees; the current 512x334 PS2 3D target still maps `_Model.mouse_x/y` using the old half-resolution `/2` conversion. The dedicated PS2 local-player draw also forces `player->lowmem = true`, which bypasses normal sequenced-model animation transforms. Fix these as separate hardware-tested changes.
...
2. Correct PS2 viewport picking coordinates so the virtual cursor, terrain click and model/loc hover tests all refer to the same rendered pixel.
3. Restore local-player walk/run/action animation without reintroducing appearance-cache/temporary-model leaks.
