...
- GS renderer finding: real-hardware testing on 2026-09-22 accepted the direct GS 3D architecture through `1332b8b19a141e1ceb0a9a28499b71f154ee15c0`. The EE still performs animation, transforms/projection, clipping, visibility, and existing depth/priority ordering, but flat/Gouraud rasterization, alpha, and perspective-correct STQ texture sampling are now performed by the GS. The first flat/Gouraud GS proof produced a significant FPS improvement; the corrected compositor restored the world/UI layering; the later STQ texture build preserved that performance improvement while real hardware showed correct-looking textures. Keep `1332b8b` as the accepted textured-GS rollback point while submission/batching experiments continue.
