...
- Last fully real-hardware-known-good integration commit: `211e94159e6ff2796de42e7b35b67f1dacd6be7a` (confirmed working on real PS2 on 2026-09-16; retains the qword-aligned 4 MiB scene arena, 32x32 / 1024-slot resident terrain window, fourteen-tile / 29x29 camera draw window and local-player model, and restores the bounded 32x32 static-loc window. It remained crash-free beyond T2852 in Lumbridge. The added loc/model workload reduced the observed live framerate from roughly 50 FPS to roughly 30 FPS; keep that performance cost visible while correctness restoration continues, with renderer/VU work treated as a separate later optimization effort.)
...
- Current policy: gameplay-first 32 MiB profile — untextured colored terrain, bounded/lazy Ground residency, fourteen-tile camera radius, local player enabled, bounded 32x32 static-loc placement enabled, capped nearby dynamic entities, simplified UI chrome, minimap off. The next isolated restoration test is normal sidebar/chat/tab UI composition while keeping software-rendered inventory item icons and interface 3D models gated by `PS2_SAFE_INTERFACE`.
