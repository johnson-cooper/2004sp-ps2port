...
- Audio finding: real-hardware testing through commit `8c8cb828513a16bd59dd4e39e09832fd75d3f8d4` proved that LIBSD + audsrv + SPU2 ADPCM playback works alongside live RuneScape networking when audsrv is built in the custom voice-only mode. The same startup beep with stock audsrv initialization left the title screen responsive but caused the later server connection to fail. The custom package removes audsrv's unused permanent PCM stream thread, stream semaphores and looping block DMA while keeping RPC, SPU2 initialization and ADPCM sample/voice playback. Preserve that voice-only distinction; title/in-game MIDI should sequence preconverted ADPCM voices rather than re-enable stock PCM streaming or EE software synthesis. This is a validated audio subsystem checkpoint, not a replacement for the accepted gameplay/performance baseline until the music path itself is hardware-tested.
