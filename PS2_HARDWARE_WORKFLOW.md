...
- Last fully real-hardware-known-good integration commit: `6d98037fd7ed7938b71c8fe8a0c5b9f9e50dcc4f` (accepted on real PS2 hardware on 2026-09-22). This includes the GS raster/texture renderer, textured and flat/Gouraud batching, and direct-GS viewport presentation. The completed 3D world no longer takes the CPU viewport-copy / RGB-to-CT16 / framebuffer-upload round trip; only sparse CPU 2D overlays are uploaded over the GS world. The user reported this build worked perfectly on hardware.
...
- GS renderer finding: real-hardware testing on 2026-09-22 accepted the direct GS architecture through `6d98037fd7ed7938b71c8fe8a0c5b9f9e50dcc4f`. The EE retains animation, transforms/projection, clipping, visibility, and existing depth/priority ordering; the GS performs flat/Gouraud rasterization, alpha, perspective-correct STQ texture sampling, batching of consecutive compatible faces without reordering, and final 3D framebuffer presentation. The CPU world viewport copy, completed-world RGB-to-CT16 conversion, and completed 3D viewport upload are removed. Keep `6d98037` as the rollback point for the final controller frame-rate experiment.
...
- Performance finding: the renderer is now GS-backed and the 3D framebuffer stays on the GS. Simulation/networking remain at the native 50 Hz. The previous 25 FPS presentation cadence is hardware-proven. The final controller setting experiment selects either full-rate 50 FPS (default) or the proven 25 FPS fallback; both are exact integer cadences of the 50 Hz loop.
