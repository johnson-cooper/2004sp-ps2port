...
- Last fully real-hardware-known-good integration commit: `64f8294d15d876c9c50107a767c8f132621facfe` (confirmed stable on real PS2 on 2026-09-20). This baseline includes the prior hardware-stable scene/memory/render work and controller-native UX, the accepted plane-transition and chatbox-readability fixes, plus the PS2 model-projection reciprocal experiment. The reciprocal path costs 16 KiB and was hardware-stable, but the first visual test showed no noticeable frame-rate improvement, so treat it as a small/neutral optimization rather than evidence that vertex projection is the dominant dense-scene bottleneck.
...
1. Preserve baseline `64f8294d15d876c9c50107a767c8f132621facfe`: qword-aligned 80x80 terrain / 72x72 loc traversal, compact scene allocations, lazy interfaces, one-slot water texture, corrected picking, animated/occluded local player, normal gameplay UI, radius-18 camera-aware visibility, controller-native UX, bounded scene rebuilds on floor/plane changes, the PS2 chatbox readability pass, and the hardware-stable model-projection reciprocal path.
