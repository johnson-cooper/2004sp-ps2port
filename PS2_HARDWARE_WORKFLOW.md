...
- Last fully real-hardware-known-good integration commit: `60fb9773199542741ea444b41b02ec68ee66554d` (confirmed stable on real PS2 on 2026-09-20, with a small but noticeable dense-scene frame-rate improvement). This baseline includes the prior hardware-stable scene/memory/render work and controller-native UX, the accepted plane-transition and chatbox-readability fixes, the hardware-stable model-projection reciprocal path, and PS2-only render-distance culling for distant inactive scenery/roof/wall-decor/ground-decor locs while preserving walls, actionable locs, collision and interactions.
...
1. Preserve baseline `60fb9773199542741ea444b41b02ec68ee66554d`: qword-aligned 80x80 terrain / 72x72 loc traversal, compact scene allocations, lazy interfaces, one-slot water texture, corrected picking, animated/occluded local player, normal gameplay UI, radius-18 camera-aware visibility, controller-native UX, bounded scene rebuilds on floor/plane changes, the PS2 chatbox readability pass, the hardware-stable model-projection reciprocal path, and accepted distant inactive-loc detail culling.
