...
- Last fully real-hardware-known-good integration commit: `99a6cb4e171898cdcef77e388856008de199cd70` (confirmed working on real PS2 on 2026-09-16; retains the qword-aligned 4 MiB scene arena, 32x32 / 1024-slot resident terrain window, fourteen-tile / 29x29 camera draw window, local-player model and bounded 32x32 static-loc window, and restores normal dynamic chat/sidebar/tab UI composition without crashes. Static loc/model rendering currently lowers observed live framerate from roughly 50 FPS to roughly 30 FPS; keep that performance cost visible while correctness restoration continues, with renderer/VU work treated as a separate later optimization effort.)
...
- Current policy: gameplay-first 32 MiB profile — untextured colored terrain, bounded/lazy Ground residency, fourteen-tile camera radius, local player enabled, bounded 32x32 static-loc placement enabled, capped nearby dynamic entities, normal dynamic 2D gameplay UI with simplified chrome, minimap off. The next isolated restoration test enables the controller virtual cursor together with inventory item icons and interface 3D model components by lifting `PS2_SAFE_INTERFACE`; watch model/icon cache pressure carefully on real hardware.
