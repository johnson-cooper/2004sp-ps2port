...
- Last fully real-hardware-known-good integration commit: `ba051b9dbcd11bfc6118bf841c677b1a48a0a943` (confirmed working perfectly on real PS2 on 2026-09-20). This baseline includes the prior hardware-stable scene/memory/render work and controller-native UX, plus the accepted PS2 plane-transition fix that rebuilds the same bounded scene when stairs/ladders change `currentLevel`, and the accepted chatbox readability pass that reuses bold12/shadowed text without adding persistent font/texture memory and falls back when an interface line would no longer fit.
...
1. Preserve baseline `ba051b9dbcd11bfc6118bf841c677b1a48a0a943`: qword-aligned 80x80 terrain / 72x72 loc traversal, compact scene allocations, lazy interfaces, one-slot water texture, corrected picking, animated/occluded local player, normal gameplay UI, radius-18 camera-aware visibility, controller-native UX, bounded scene rebuilds on floor/plane changes, and the PS2 chatbox readability pass.
